...
def visualize(x, z_pres, z_where_scale, z_where_shift, rbox=rbox, gbox=gbox, num_obj=8 * 8):
    ...
    # z_scale = z_where[:, :, :2].reshape(-1, 2)
    # z_shift = z_where[:, :, 2:].reshape(-1, 2)
    ...
    bbox = spatial_transform(z_pres * gbox + (1 - z_pres) * rbox,
                             ...
                             inverse=True)
    bbox = (bbox + torch.stack(num_obj * (x,), dim=1).reshape(-1, 3, *img_shape)).clamp(0.0, 1.0)
    return bbox
...
def add_bbox(x, score, z_where_scale, z_where_shift, rbox=rbox, gbox=gbox, num_obj=8 * 8):
    ...
    bbox = spatial_transform(score * gbox + (1 - score) * rbox,
                             ...
                             torch.Size([bs * num_obj, 3, *img_shape]),
                             inverse=True)
    bbox = (bbox + x.repeat(1, 3, 1, 1).reshape(-1, 3, *img_shape)).clamp(0.0, 1.0)
    return bbox
...
def bbox_in_one(x, z_pres, z_where_scale, z_where_shift, gbox=gbox):
    ...
    z_scale = z_where_scale.reshape(-1, 2)
    z_shift = z_where_shift.reshape(-1, 2)
    # argmax_cluster = argmax_cluster.reshape(-1, 1, 1, 1)
    # kbox = boxes[argmax_cluster.reshape(-1)]
    bbox = spatial_transform(z_pres * gbox,  # + (1 - z_pres) * rbox,
                             torch.cat((z_scale, z_shift), dim=1),
